...
module.exports.login = function(req, res) {
    ...
    if (validatorResponse) {
        async.waterfall([
                function(next) {
                    usersModel.getUserDetails(validatorResponse.phone, res, res, next); //Get login user details
                },
...
function validatePassword(password, userDetails, req, res, next) {
    password = crypto.pbkdf2Sync(password, userDetails.salt, 1000, 32, 'sha256').toString('hex');
